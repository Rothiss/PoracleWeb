...
function get_all_forms($pokemon_id)
{
  global $monsters_json;
  $json = json_decode($monsters_json, true);
  $form_exclude = ["Shadow", "Purified", "Copy 2019", "Fall 2019", "Spring 2020", "Vs 2019"];
  $forms = [];
  $forms[0] = "All";
...
function get_grunt($type, $gender)
{
  ...
  foreach ($json as $key => $value) {
    if (strtoupper($value['type']) == strtoupper($type) && $value['gender'] == $gender) {
      ...
      if (strtoupper($value['type']) == strtoupper($type) && $gender == 0) {
        return $key;
      }
    }
  }
    ...
    if (is_array($value)) {
      foreach ($value as $key => $value2) {
        ...
        if ("$key" == "city_district") {
          $city_district = $value2;
        }
        if ("$key" == "country_code") {
          $country = strtoupper($value2);
        }
      }
    }
    if (@$city_district <> "") {
...
function default_distance($table)
{
  ...
  if (isset($_SESSION['id'])) {
    ...
    $result = $conn->query($sql) or die(mysqli_error($conn));
    while ($row = $result->fetch_assoc()) {
      $default_distance = $row['distance'];
    }
  }

  if (!isset($default_distance)) {
    $default_distance = 0;
  }
  if ($default_distance == 0 && @$disable_areas == "True") {
    $default_distance = $_SESSION['defaultDistance'];
  };
